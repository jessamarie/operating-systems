	...
	public void run(Statistics rr) {
		...
		while (!isFinished(processes)) {
			...
			if (!readyQueue.isEmpty()) {
				...
				if( elapsedTime == 4 ){
					arrivalTime = 4;
				} else if (p.getReturnTime() == p.getInitalArrivalTime()) {		
					arrivalTime = p.getInitalArrivalTime();	
				} else {
				    arrivalTime = p.getReturnTime() + t_cs; /* Don't include load/unload times */
				}
				
				performBurst(p);
				
				p.setWaitTime(p.getWaitTime() + (p.getStartTime() - arrivalTime));
				...
				while ( removeFinishedProcesses() == false && 
